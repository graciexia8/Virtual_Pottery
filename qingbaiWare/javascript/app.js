...
window.InitDemo = async function() {
	try {
		const vertexShader = await loadTextResource("./../qingbaiWare/Shaders/shader.vs.glsl");
		const fragmentShader = await loadTextResource("./../qingbaiWare/Shaders/shader.fs.glsl");
		const modelData = await loadJSONResource("./../models/json_models/qingbaiWare.json");
