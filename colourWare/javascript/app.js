...
window.InitDemo = async function() {
	try {
		const vertexShader = await loadTextResource("./../colourWare/Shaders/shader.vs.glsl");
		const fragmentShader = await loadTextResource("./../colourWare/Shaders/shader.fs.glsl");
		const modelData = await loadJSONResource("./../models/json_models/colourWare.json");
