...
window.InitDemo = async function() {
	try {
		const vertexShader = await loadTextResource("./../neolithic/Shaders/shader.vs.glsl");
		const fragmentShader = await loadTextResource("./../neolithic/Shaders/shader.fs.glsl");
		const modelData = await loadJSONResource("./../models/json_models/neolithicCeramic.json");
