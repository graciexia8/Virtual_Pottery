...
window.InitDemo = async function() {
	try {
		const vertexShader = await loadTextResource("./../cizhouWare/Shaders/shader.vs.glsl");
		const fragmentShader = await loadTextResource("./../cizhouWare/Shaders/shader.fs.glsl");
		const modelData = await loadJSONResource("./../models/json_models/cizhouWare.json");
